...
Your self-hosted instance exposes the same external surface as the hosted service: a [REST API](/rest-api/) and an [MCP server](/mcp-server/) under your backend's `/api` path. Connect MCP clients over [OAuth](/oauth-connect/) (preferred) — note that OAuth requires Orcha to be served over HTTPS, which the Traefik setup above already provides. Coding agents without browser OAuth can use the [alternative: a Personal Access Token](/mcp-server/#the-alternative-personal-access-token).
